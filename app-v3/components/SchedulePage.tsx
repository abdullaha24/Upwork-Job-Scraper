...
export function SchedulePage({ settings, onChange }: Props) {
	...
	const minutesToTimeString = (totalMinutes: number): string => {
		const clamped = Math.min(
			1439,
			Math.max(0, Math.floor(Number.isFinite(totalMinutes) ? totalMinutes : 0)),
		);
	...
	return (
		<Box className="page-shell">
			...
			<Callout.Root mb="4" size="2" variant="soft" color="blue">
				...
				<Callout.Text>
					<strong>How to use scheduler settings:</strong> choose how often to
					run the scraper in minutes, then select the days when scraping is
					allowed. Set a daily start and end time using the slider to limit
					scraping to that window in your local time zone. The scraper runs only
					when all three rules match: interval, active day, and time window.
				</Callout.Text>
			</Callout.Root>

			<Card className="surface-card" mb="4">
				<Text size="2" weight="medium" mb="3" as="p">
					Run the scraper every ...
				</Text>
			...
			<Card className="surface-card">
				...
				<Flex gap="4" mb="3" wrap="wrap">
					<Text size="1" color="gray" as="p">
						Start time: {formatTimeLabel(startMinutes)}
					</Text>
					<Text size="1" color="gray" as="p">
						End time: {formatTimeLabel(endMinutes)}
					</Text>
				</Flex>
				<Slider
					min={0}
					...
					onValueChange={(value) => {
						...
						const nextEnd = Math.min(
							1439,
							Math.max(nextStart + 1, Math.floor(value[1])),
						);
						onChange({
							...settings,
							timeWindow: {
